...
interface OurConclusionsSectionProps {
  data: unknown;
}

/**
 * Render our team conclusions - handles various formats
 */
function renderOurConclusions(data: unknown) {
  if (!data) {
    return null;
  }
...
export function OurConclusionsSection({ data }: OurConclusionsSectionProps) {
  const content = renderOurConclusions(data);
  ...
  return (
    <SectionCard title="Выводы нашей команды" icon={<Users className="h-5 w-5" />}>
      {content}
    </SectionCard>
  );
}
